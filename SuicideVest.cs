...
{
    [Info("Suicide Vest", "Arkanium", "2.1.1")]
    ...
    public class SuicideVest : RustPlugin
    {
        ...
        private object CanMoveItem(Item item)
        {
            ...
            if (!_armed.Contains(player) || !_data.Vests.Contains((uint)item.uid.Value)) return null;
        ...
        private IEnumerable<Item> GetEntityItems()
        {
            ...
            {
                var item = entity.GetComponent<Item>(); // Essayer d'obtenir un composant d'objet
                if (item != null)
                {
                    string itemUidString = item.uid.ToString();
                    uint itemUid;
                    if (uint.TryParse(itemUidString, out itemUid))
                    {
                        if (_data.Vests.Contains(itemUid)) // Comparer les chaînes d'identifiants d'objets
                        {
                            returnList.Add(item);
                        }
                    }
        ...
        private void Explode(BasePlayer player)
        {
            ...
            if (vest != null) 
            {
                uint vestUid = (uint)vest.uid.Value;
                _data.Vests.Remove(vestUid);
            }
            SaveData();
        }
        ...
        {
            return player.inventory.containerWear.itemList.Find(x => _data.Vests.Contains((uint)x.uid.Value)) != null;
        }

        private Item GetVest(BasePlayer player)
        {
            return player.inventory.containerWear.itemList.Find(x => _data.Vests.Contains((uint)x.uid.Value));
        }
        ...
        private void ConsoleCmd(ConsoleSystem.Arg arg)
        {
            ...
            _data.Vests.Add((uint)vestItem.uid.Value);
            SaveData();
        }
        ...
        private void SetupItems()
        {
            ...
            foreach (var item in list.Where(x => _data.Vests.Contains((uint)x.uid.Value)))
            {
        ...
        private void Cleanup()
        {
            ...
            for (var z = 0; z < _data.Vests.Count; z++)
            {
                ...
                if (list.FirstOrDefault(x => (uint)x.uid.Value == vest) == null) _data.Vests.Remove(vest);
            }
        ...
        {
            if (action != "drop" || !_data.Vests.Contains((uint)item.uid.Value) || !_armed.Contains(player)) return null;
            SendReply(player, lang.GetMessage("CannotMoveWhilstArmed", this, player.UserIDString));
            return false;
        }
